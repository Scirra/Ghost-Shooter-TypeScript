...
function CheckBulletHitMonster(bulletInstance, runtime) {
    ...
    for (const monsterInstance of runtime.objects.Monster.instances()) {
        ...
        if (bulletInstance.testOverlap(monsterInstance)) {
            ...
            const explosionInstance = Explosion.createInstance("Main", bulletInstance.x, bulletInstance.y);
...
export function OnMouseDown(e, runtime) {
    ...
    // their gun.
    const bulletInstance = runtime.objects.Bullet.createInstance("Main", playerInst.getImagePointX(1), playerInst.getImagePointY(1));
